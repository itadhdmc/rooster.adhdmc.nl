import { signInWithGoogle } from '../lib/auth'
import { ALLOWED_DOMAIN } from '../lib/supabase'

export default function Login() {
  async function handleLogin() {
    try {
      await signInWithGoogle()
    } catch (err) {
      alert('Inloggen mislukt. Probeer opnieuw.')
      console.error(err)
    }
  }

  return (
    <div className="min-h-screen flex" style={{ backgroundColor: '#f2f2f7' }}>
      {/* Left panel – branding */}
      <div
        className="hidden lg:flex flex-col justify-between w-1/2 p-12"
        style={{ backgroundColor: '#3c3c3b' }}
      >
        <img src="/logo.png" alt="ADHDMC" className="h-12 w-auto" />
        <div>
          <h2 className="text-4xl font-bold text-white leading-snug mb-4">
            Roosterbeheer<br />voor de studentenpool
          </h2>
          <p className="text-white/50 text-lg leading-relaxed">
            Plan diensten, geef je beschikbaarheid door en synchroniseer met Google Agenda.
          </p>
        </div>
        <p className="text-white/30 text-sm">
          ADHD Medisch Centrum &copy; {new Date().getFullYear()}
        </p>
      </div>

      {/* Right panel – login form */}
      <div className="flex flex-1 items-center justify-center p-8">
        <div className="w-full max-w-sm">
          {/* Mobile logo */}
          <div className="lg:hidden mb-10 text-center">
            <img src="/logo.png" alt="ADHDMC" className="h-14 w-auto mx-auto mb-3" />
            <p className="text-sm text-gray-500">Zorgadministratie Rooster</p>
          </div>

          <div className="bg-white rounded-2xl shadow-lg p-8">
            <h1 className="text-2xl font-bold text-dark mb-1">Inloggen</h1>
            <p className="text-gray-500 text-sm mb-8">
              Gebruik je <strong>@{ALLOWED_DOMAIN}</strong> account
            </p>

            <button
              onClick={handleLogin}
              className="w-full flex items-center justify-center gap-3 border-2 border-gray-200 hover:border-salmon-400 bg-white hover:bg-salmon-50 transition-all rounded-xl px-4 py-3.5 font-semibold text-dark text-sm shadow-sm"
            >
              <GoogleIcon />
              Doorgaan met Google Workspace
            </button>

            <p className="text-xs text-gray-400 text-center mt-6 leading-relaxed">
              Alleen toegankelijk voor medewerkers van ADHDMC.<br />
              Problemen? Neem contact op met de beheerder.
            </p>
          </div>
        </div>
      </div>
    </div>
  )
}

function GoogleIcon() {
  return (
    <svg className="w-5 h-5 flex-shrink-0" viewBox="0 0 24 24">
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
    </svg>
  )
}
